namespace HGE.IO.AudioEngine.sidPlayLib.common
{
    public static class SIDEndian
    {
        ...
        /// <param name="word"></param>
        public static void endian_16(byte[] ptr, int pos, int word)
        {
            ptr[pos + 0] = (byte)endian_16lo8(word);
            ptr[pos + 1] = (byte)endian_16hi8(word);
        }

        public static void endian_16(char[] ptr, int pos, int word)
        {
            byte[] newptr = new byte[] { (byte)ptr[pos + 0], (byte)ptr[pos + 1] };
