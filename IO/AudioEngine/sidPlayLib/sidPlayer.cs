...
using HGE.IO.AudioEngine.sidPlayLib.components.mem;
using HGE.IO.AudioEngine.sidPlayLib.components.mos6510;
using HGE.IO.AudioEngine.sidPlayLib.components.mos6526;
using HGE.IO.AudioEngine.sidPlayLib.components.mos656x;
using HGE.IO.AudioEngine.sidPlayLib.components.sid;
using HGE.IO.AudioEngine.sidPlayLib.components.sid.resid;
using HGE.IO.AudioEngine.sidPlayLib.components.sid.xsid;
using HGE.IO.AudioEngine.sidPlayLib.components.sidtune;

namespace HGE.IO.AudioEngine.sidPlayLib
{
    internal class sidPlayer
    {
        ...
        private delegate long OutputDelegate(byte[] buffer, int off);

        private OutputDelegate output;

        // 8 bit sound output generation routines

        internal long monoOut8MonoIn(byte[] buffer, int off)
        {
            buffer[off] = ((byte)(sid.output((short)8) ^ 0x80));
            return 1;
        }

        internal long stereoOut8MonoIn(byte[] buffer, int off)
        {
            var sample = ((byte)(sid.output((short)8) ^ 0x80));
            buffer[off + 0] = sample;
            buffer[off + 1] = sample;
            return 2;
        }
        ...
        internal long monoOut16MonoIn(byte[] buffer, int off)
        {
            SIDEndian.endian_16(buffer, off, (int)sid.output((short)16));
            return 2;
        }

        internal long stereoOut16MonoIn(byte[] buffer, int off)
        {
        ...
        public sidPlayer(BinaryReader reader)
        {
        ...
        public long play(byte[] buffer, int length)
        {
            inPlay = true;
